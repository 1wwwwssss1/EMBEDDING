...
def get_l2_name(dept: str) -> str:
    """路径格式：百运网/一级部门/二级部门/...，取 parts[2]；不存在则返回空"""
    if not dept:
        return ""
    parts = [p.strip() for p in dept.split("/") if p.strip()]
...
def generate_strict_report():
    ...
    try:
        ...
        for l1 in L1_ORDER:
            ...
            # 无二级部门的（平台中心、财务部、人力行政部）
            if l1 in ("平台中心", "财务部", "人力行政部"):
                call_users = l1_df['uid'].nunique()
                call_total = len(l1_df)
                headcount = cfg['total']
                user_pct = f"{round(call_users/headcount*100, 1)}%" if headcount > 0 else "-%"
                call_pct = f"{round(call_total/total_calls*100, 1)}%" if total_calls > 0 else "-%"
                md += f"| 一级部门 | 负责人 | 二级部门 | 部门人数 | 调用功能人数 | 功能调用总次数 | 使用人数占比 | 功能调用占比 |\n"
                md += f"| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n"
                md += f"| {l1} | {mgr_str} | — | {headcount} | {call_users} | {call_total} | {user_pct} | {call_pct} |\n\n"

                # 人员TOP10
                ut = l1_df.groupby('name').size().sort_values(ascending=False).head(10)
                if len(ut) > 0:
                    md += f"{l1}人员TOP10：\n\n"
                    md += "| 姓名 | 次数 |\n| :--- | :--- |\n"
                    for name, cnt in ut.items():
                        md += f"| {name} | {cnt} |\n"
                    md += "\n"
            else:
                # 各一级部门下的完整二级部门列表
                ...
                # 未分配到二级部门的行：显示负责人姓名，人数为未分配人数
                other_df = l1_df[l1_df['l2'] == ""]
                # 未分配人数 = 一级部门总人数 - 所有二级部门人数之和
                ...
                if len(other_df) > 0:
                    ...
                    o_total = len(other_df)
                    o_user_pct = f"{round(o_users/other_headcount*100, 1)}%" if other_headcount > 0 else "-%"
                    o_call_pct = f"{round(o_total/total_calls*100, 1)}%" if total_calls > 0 else "-%"
                    md += f"| {l1} | {mgr_str} | {mgr_str}（直属，{other_names}） | {other_headcount} | {o_users} | {o_total} | {o_user_pct} | {o_call_pct} |\n"

                for l2 in l2_list:
                    l2_df = l1_df[l1_df['l2'] == l2]
                    headcount = L2_HEADCOUNT.get(l2, 0)
                    call_users = l2_df['uid'].nunique()
                    call_total = len(l2_df)
                    user_pct = f"{round(call_users/headcount*100, 1)}%" if headcount > 0 else "-%"
                    call_pct = f"{round(call_total/total_calls*100, 1)}%" if total_calls > 0 else "-%"
                    md += f"| {l1} | {mgr_str} | {l2} | {headcount} | {call_users} | {call_total} | {user_pct} | {call_pct} |\n"

                md += "\n"

                # 每个有调用记录的二级部门输出人员TOP10
                for l2 in l2_with_data:
                    l2_df = l1_df[l1_df['l2'] == l2]
                    ut = l2_df.groupby('name').size().sort_values(ascending=False).head(10)
                    if len(ut) == 0:
                        continue
                    md += f"{l2}人员TOP10：\n\n"
                    md += "| 姓名 | 次数 |\n| :--- | :--- |\n"
